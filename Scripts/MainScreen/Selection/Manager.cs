...
internal class Manager
{
	...
	/// <summary>
	/// Should only have 3 refs. one to NULL in the event raiser,<br/>
	/// one in SelectConnection() and one for the SelectedConnection property
	/// </summary>
	...
	private void RaiseSelectionChanged()
	{
		...
		if (selectedMapPiece is not null && addon.MS.Map.GetConnection(SelectedMapPiece.MainSection, SelectedMapPiece.Coord, out SectionConnection conn))
		{
	...
	private void UpdateGizmos(object sender, EventArgs e)
	{
		...
		// Insert Diamond on path target map piece
		...
		if (selectedMapPiece is not null && selectedMapPiece2nd is not null)
		{
			...
			if (!query.IsValid)
			{
				GD.PushError($"Manager::UpdateGizmos() query was invalid!");
				return;
			}
	...
	internal void ClearSelection()
	{
		...
		if (mainScreen.Mode != VIEWERMODE.SECTION) { selectedSectionResource = null; }
	...
	internal void SelectMapPiece(MapPiece mapPiece, bool runUpdates = true)
	{
		...
		selectedMapPiece2nd = null; // NOTE Do we have to?
		SelectSection(mapPiece.MainSection, false);
		if (runUpdates)
		{
			RaiseSelectionChanged();
			mainScreen.RaiseUpdateUI();
	...
	AddGizmoForAnswer(PathAnswer answer)
	{
		...
		AddGizmoForSectionPath(answer.startConnection, answer.endConnection, answer.connectionPath, Colors.LimeGreen);
	}
	...
	private void AddGizmoForPath(List<PathLocation> path, Color col)
	{
		if (path.Count > 0)
		{
			SegmentedGizmo pathLine = GizmoSegmented.Instantiate() as SegmentedGizmo;
			pathLine.pathScale = 1.0f;
			pathLine.offset = Vector3.Up * 0.5f;
			pathLine.color = col;
			pathLine.ClearSegments();
			pathLine.AddSegments(path);
			addon.MS.Gizmos.AddChild(pathLine);
			pathLine.Position = Vector3.Zero;
		}
	}
